...
function App() {
  ...
  const searchUsers = async (form) => {
    setIsLoading(true);
    const users = await githubRequest(
      `https://api.github.com/search/users?q=${form.query}`
    );
  ...
  let resultText;
  if (searchResults) {
    resultText = searchResults.length
  ...
  return (
    <main>
      ...
      <UserDetails user={currentUser} onClose={() => setCurrentUser("")} />
    </main>
  );
}
